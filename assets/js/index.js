...
window.addEventListener('load', function() {
    ...
    document.querySelector('#ImgSelector button').addEventListener('click', function() {
        ...
        parseHTML("https://beginworld.website-us-east-1.linodeobjects.com/memes.html", process.env.SCRAPPER_KEY);
    });
    ...
    document.querySelector('input[type="file"]').addEventListener('change', function() {
        if (this.files && this.files[0]) {
            ...
            uploadImg(this.files[0], process.env.IMGUR_KEY, onComplete);
        }
    });
});
